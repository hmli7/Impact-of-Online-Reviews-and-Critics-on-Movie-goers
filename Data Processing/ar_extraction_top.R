...
#sub top
ar.rate <- subset(ar.rate, ar.rate$SR==1)
...
describe(ar.aggr)
write.csv(ar.aggr,file.path(path.output,"ar_aggr_8_SRcombination.csv"),row.names = F)
str(ar.aggr)

# dealing missing value -> math cum after combination
ar.aggr.sub <- subset(ar.aggr,select = c("MID","Date","volume_day","sum_rate","count_rate","count_35"))
ar.aggr.sub <- ar.aggr.sub[order(ar.aggr.sub$MID,ar.aggr.sub$Date),]
###no missing value so ignore impute process
ar.aggr.sub_ad <- ar.aggr.sub
...
write.csv(ar.aggr.sub_ad,file.path(path.output,"ar_aggr_9_SRcum.csv"),row.names = F)
#change var names to top
fix(ar.aggr.sub_ad)
describe(ar.aggr.sub_ad)
str(ar.aggr.sub_ad)
detach(ar.aggr.sub_ad)

#combine sr table and normal table
ar.aggr.normal <- fread(file.path(path.output,"ar_aggr_7_mathscore.csv"))
str(ar.aggr.normal)
ar.aggr.whole <- full_join(ar.aggr.normal,ar.aggr.sub_ad,by=c("MID","Date"))
str(ar.aggr.whole)
describe(ar.aggr.whole)
write.csv(ar.aggr.whole,file.path(path.output,"ar_aggr_10_whole.csv"),row.names = F)
