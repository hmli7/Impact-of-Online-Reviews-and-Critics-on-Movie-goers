...
write.csv(cr.aggr,file.path(path.output,"cr_aggr_whole_4_timeextand_1.csv"),row.names = F)
describe(cr.aggr)
#normal
attach(cr.aggr)
...
cr.aggr <- data.frame(cr.aggr,Tomatometer_label = Tomatometer_label$Tomatometer_label)
fix(cr.aggr)
describe(cr.aggr)
rm(aggr.countfresh,aggr.countrate,aggr.countwhole,aggr.cumsum)
cr.aggr <- impute(cr.aggr,0)
write.csv(cr.aggr,file.path(path.output,"cr_aggr_whole_4_timeextand_2_addcum.csv"),row.names = F)
