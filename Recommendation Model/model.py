...
def gradAscent(data,ar,cr, K):
	dataMat = mat(data)
	arMat = mat(ar)
	crMat = mat(cr)
	print dataMat,arMat,crMat
	m, n = shape(dataMat)
		...
		for i in xrange(m):
			for j in xrange(n):
				if dataMat[i,j] > 0:
					...
					for k in xrange(K):
						pq = pq + p[i,k]*q[k,j]
					error = (1-W[i,0]-M[i,0])*pq +  W[i,0] * arMat[i,j] + M[i,0] * crMat[i,j]
					error = dataMat[i,j] - error
					for k in xrange(K):
						p[i,k] = p[i,k] - alpha * (2 * error * (-1+W[i,0]+M[i,0]) * q[k,j] + beta * p[i,k])
						q[k,j] = q[k,j] - alpha * (2 * error * (-1+W[i,0]+M[i,0]) * p[i,k] + beta * q[k,j])
		...
		for i in xrange(m):
			for j in xrange(n):
				if dataMat[i,j] > 0:
					...
					error = (1-W[i,0]-M[i,0])*error +  W[i,0] * arMat[i,j] + M[i,0] * crMat[i,j]
					loss = loss + (dataMat[i,j] - error) * (dataMat[i,j] - error)
					for k in xrange(K):
						loss = loss + beta * (p[i,k] * p[i,k] + q[k,j] * q[k,j]) / 2
					loss = loss + beta * (W[i,0] * W[i,0] + M[i,0] * M[i,0]) / 2
...
if __name__ == "__main__":
	...
	for i in xrange(m):
		for j in xrange(n):
			dataMat[i,j] = (1-W[i,0]-M[i,0]) * dataMat[i,j] +  W[i,0] * arMat[i,j] + M[i,0] * crMat[i,j]
	result = dataMat
	#print p
	#print q
	...
	numpy.savetxt("ar_w.csv", W, delimiter = ',');
	numpy.savetxt("cr_m.csv", M, delimiter = ',');
