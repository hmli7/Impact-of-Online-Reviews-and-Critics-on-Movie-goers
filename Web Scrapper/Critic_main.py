...
ssl._create_default_https_context = ssl._create_unverified_context #Python 升级到 2.7.9 之后引入了一个新特性，当使用urllib.urlopen打开一个 https 链接时，会验证一次 SSL 证书，全局取消证书验证
...
target2 = open(os.path.join(Parentpath,'Data\Critics_review.txt'), 'a') # 设置写入文件 
...
#print startPageBox
startPage = int(startPageBox[-3])#从record文件中找到最后的记录，并设置爬虫程序起始页；由于电影名中有逗号，所以倒数取值
pageInd = startPage
timeout = 600
while 1:
    ...
    try:
        for line in box[startPage:]:
            ...
            page=0
            docnumber_count=0
            max_page=Crget.whole_pagenumber(RReviewURL)
            ...
            while page < max_page:
                page=page+1
                pageurl=str(RReviewURL+"?page="+str(page)+"&sort=")
                failtime=0
                while 1:
                    try:
                        print '# Loading: ' + pageurl
                        content=Crget.get_code(pageurl)                       
                        failtime=10
                    except:
                        print "## fail"
                        failtime= failtime+1
                ...
                line = Crget.get_review(content)
                print "## Start printinfo"
                for item in line:
                    docnumber_count = docnumber_count+1
                    target.write(Crget.get_cID(item)+',')
                    target.write(Crget.get_cName(item)+',')
                    target.write(Crget.get_comName(item)+',')
                    target.write(mID+',')
                    target.write(Crget.get_tomatoLogo(item)+',')
                    target.write(Crget.get_topCritic(item)+',')
                    target.write(Crget.get_date(item)+',')
                    target.write(Crget.get_origScore(item)+',')
                    target.write('\n') 
                    
                    target2.write(Crget.get_cID(item)+',')
                    target2.write(Crget.get_comName(item)+',')
                    target2.write(mID+',')
                    target2.write(Crget.get_link(item)+',')
