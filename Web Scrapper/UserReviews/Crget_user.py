...
##<a class="bold unstyled articleLink" href="/user/id/956694313/"><span style="word-wrap:break-word"> Ezra S</span> </a>"
    ...
    reStucName = " ".join(re.split('\W+',str(cName[0])))
    if len(cName) == 0:
        return 'Null'
    # elif re.compile('\s$').search(str(cName[0])) or re.compile('"').search(str(cName[0])):
    #     splitname = re.split('\W+',str(cName[0]))
    #     return '"' + str(splitname[1]) + '"'
    # elif len(re.split('\W+',str(cName[0]))) <= 2:
    #     return " ".join(re.split('\W+',str(cName[0])))
    # else:
    #     return '"' + str(cName[0]) + '"'
    elif reStucName == ' ':
        return 'Null'
...
#type 4: want to see:<img src="https://d2a5cgar23scu2.cloudfront.net/static/images/rating/wts.png"/>
    ...
    rateDecpattern = re.compile('(frac12)')
