...
source = open(os.path.join(filepath,'Data Source\MovienIndex\MovieIndex2-00-11_Del-1+repeat.csv'),'r')
...
record = open(os.path.join(filepath,'Data\\UserReviewsData\\UserReviewRecords.csv'), 'a+')#保存成功扒取的电影记录
target = open(os.path.join(filepath,'Data\\UserReviewsData\\UserReviews.txt'), 'a') # 设置写入文件
target2 = open(os.path.join(filepath,'Data\\UserReviewsData\\UserTable.txt'), 'a') # 设置写入文件 
failfile = open(os.path.join(filepath,'Data\\UserReviewsData\\UserReviews_FailPage.csv'), 'a')
